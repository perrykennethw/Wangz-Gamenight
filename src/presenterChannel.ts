...
export type PresentationState = LobbyPresentation | FeudPresentation | SpinPresentation
...
function postState(channel: BroadcastChannel, state: PresentationState): void {
  const message: PresentationMessage = { version: 1, roomCode: state.code, kind: 'state', state }
  channel.postMessage(message)
}

function isPresentationMessage(value: unknown, roomCode: string): value is PresentationMessage {
  if (!value || typeof value !== 'object') return false
  const message = value as Partial<PresentationMessage>
  return message.version === 1 && message.roomCode === roomCode && (message.kind === 'request' || message.kind === 'state')
}
...
export function createFeudPresentation(input: FeudBoardInput): FeudPresentation {
  const activePlayer = input.room.participants.find((participant) => participant.id === input.room.playPass.activePlayerId)
  return {
    mode: 'feud',
    ...
    question: {
      answers: input.question.answers.map(({ id, label, points }, index) => input.revealed.includes(index)
        ? { id, label, points }
        : { id, label: '', points: 0 }),
    },
    ...
    buzzer: {
      ...
      winner: input.room.buzzer.winner
        ? { playerName: input.room.buzzer.winner.playerName, team: input.room.buzzer.winner.team }
        : null,
    },
    decision: {
...
export function createSpinPresentation(room: RoomSnapshot): SpinPresentation | null {
...
export function presenterRoomCode(): string | null {
  const code = new URLSearchParams(window.location.search).get('present')?.trim().toUpperCase() ?? ''
...
export function usePresentationPublisher(state: PresentationState | null): void {
  ...
  useEffect(() => {
    ...
    channel.onmessage = (event: MessageEvent<unknown>) => {
      if (isPresentationMessage(event.data, state.code) && event.data.kind === 'request' && stateRef.current) postState(channel, stateRef.current)
    }
...
export function usePresentation(roomCode: string): PresentationState | null {
  ...
  useEffect(() => {
    ...
    const request = () => {
      const message: PresentationMessage = { version: 1, roomCode, kind: 'request' }
      channel.postMessage(message)
    }
    channel.onmessage = (event: MessageEvent<unknown>) => {
      if (isPresentationMessage(event.data, roomCode) && event.data.kind === 'state') setState(event.data.state)
    }
    request()
